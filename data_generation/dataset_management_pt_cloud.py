                    ...
                    if len(cameras) > 0:
                        for cam in cameras:
                            ...
                            for t in range(dem_len):
                                ...
                                #print("cam_point_cloud : "+str(cam_pt_cl))
...
def init_worker(visited_states_, tsk, cam, ex, var, colors, mask_colors_to_obj_names, obj_poses, traj_obj_poses, objects_strId_to_intId, mode, cam_transf):
...
def process_one_ts_data_to_pose_from_pt_cloud(t):
    global _visited_states_
    global _tsk
    global _cam
    global _ex
    ...
    ts = wp_hist[t] if _visited_states_ == 'wp_only' else t
    concat_obj_infos = np.array([])
    segm_path = path_dataset + _mode + "/" + str(_tsk) + "/obs_segmentations/" + str(_cam) + "/" + str(
        _var) + "/" + str(_ex) + "_" + str(t) + ".png"
    numpymask = np.expand_dims(np.mean(cv2.imread(segm_path), axis=-1).astype(np.uint8), -1)

    pt_cl_path = path_dataset + _mode + "/" + str(_tsk) + "/pt_clouds/" + str(_cam) + "/" + str(_var) + "/" + str(_ex) + "_" + str(t) + ".npy"
    pt_cloud = np.load(pt_cl_path)


    color_in_segm = np.unique(numpymask)
    print("_colors : "+str(_colors))
    print("color_in_segm.astype('str') : "+str(color_in_segm.astype('str')))


    visible_objects = [_mask_colors_to_obj_names[c] for c in color_in_segm.astype('str') if c in list(_colors)]
    obj_tab = {}
    for obj in _obj_poses[str(ts)]:
        if obj["id"] in visible_objects:
            normed_pose = np.array(obj["bbox"])
            obj_colors_in_mask = np.array([color for color, obj_name in _mask_colors_to_obj_names.items() if obj_name == obj["id"]]).astype(int)
            #print("numpymask.shape : "+str(numpymask.shape))
            indices = np.where(np.any(numpymask == obj_colors_in_mask, axis=-1))
            obj_im = np.zeros_like(numpymask)
            obj_im[indices]=255
            obj_im = np.repeat(obj_im, 3, axis=-1)
            #print('indices : '+str(indices))
            #obj_mask = numpymask[np.argwhere(numpymask==obj_colors_in_mask.any())]
            plt.imsave(obj["id"]+str(".png"), obj_im)
            pt_cl_obj = pt_cloud[indices]
            #print("pt_cl_obj.shape : "+str(pt_cl_obj.shape))
            obj_tab[obj["id"]]={"2d_points":indices, "3d_points":pt_cl_obj, "int_id":_objects_strId_to_intId[obj["id"]]}
...
def process_one_ts_data_to_pose_from_3D_full_pose(t):
    ...
    for obj in _obj_poses[str(ts)]:
        ...
        if obj["id"] in visible_objects:
            ...
            # obj_tab.append(np.concatenate([np.array([_objects_strId_to_intId[obj["id"]]]), np.array(box_2d), normed_pose], axis=-1))
            obj_tab.append(
                np.concatenate([np.array([_objects_strId_to_intId[obj["id"]]]), np.array(box_2d), obj_center, obj_dims, quat], axis=-1))
            # print(_traj_obj_poses[t, n_obj])
            """print(box_2d)
            if t==0:
                plt.imshow(numpymask)
...
def export_dataset_to_pose_estimation_data(path_dataset, config, use_3D_gt=True, action_range="next_ts", visited_states= 'all', goal=None, keep_gripper_close_states=False):
    pt_cl_dataset = h5py.File(str(path_dataset) + '_pt_cl_keypoints.hdf5', 'w')
    ...
    for mode in ['train']:
        ...
        if len(tasks) > 0:
            ...
            for tsk in tasks:
                tsk_group = pt_cl_dataset.create_group(str(tsk))
                tsk_group.attrs["max_seq_len"] = 0
                tsk_group.attrs["max_num_points"] = 0
                ee_maxs = np.load(path_dataset + "train/" + str(tsk) + "/ees_maxs.npy")
                ee_mins = np.load(path_dataset + "train/" + str(tsk) + "/ees_mins.npy")
                obj_pose_maxs = np.load(path_dataset + "train/" + str(tsk) + "/obj_pose_maxs.npy")
                obj_pose_mins = np.load(path_dataset + "train/" + str(tsk) + "/obj_pose_mins.npy")
                ...
                for var in range(config["n_variation_per_task"]):
                    print('var ' + str(var))
                    for ex in range(config["n_ex_per_variation"]):
                        ex_group = tsk_group.create_group(str(ex))
                        print('ex ' + str(ex))
                        ...
                        #print(obj_poses)
                        obj_tab = {}
                        overall_point_clouds = {}
                        tsk_group.attrs["max_seq_len"] = max(tsk_group.attrs["max_seq_len"], seq_len)
                        for cam in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                            cam_group = ex_group.create_group(cam)
                            #obj_data = obs_grp.create_dataset("object", (seq_len, n_obj_per_img * low_dim), dtype='f')
                            ...
                            with Pool(initializer=init_worker, initargs=(visited_states_, tsk, cam, ex, var, colors, mask_colors_to_obj_names, obj_poses, traj_obj_poses, objects_strId_to_intId, mode, cam_transf, )) as pool:
                                ...
                                res = pool.map(process_one_ts_data_to_pose_from_pt_cloud, range(seq_len))
                                # res <-- seq_len*{"2d_points":indices, "3d_points":pt_cl_obj, "int_id":_objects_strId_to_intId[obj["id"]]}
                                obj_tab[cam]=res
                            t=0
                            for frame_ in res:
                                if not (t in overall_point_clouds.keys()):
                                    overall_point_clouds[t]={}
                                #frame_pt_cl = cam_group.create_dataset(str(t), (max_nb_obj_per_img, n_pts, pt_low_dim), dtype='f')(cam)
                                frame_group = cam_group.create_group(str(t))

                                for i, (k, obj_) in enumerate(frame_.items()):
                                    #print(obj_)
                                    if not (obj_["int_id"] in overall_point_clouds[t].keys()):
                                        overall_point_clouds[t][obj_["int_id"]] = []

                                    overall_point_clouds[t][obj_["int_id"]].append(obj_["3d_points"])
                                    overall_point_clouds[t][obj_["int_id"]] = [np.unique(np.concatenate(overall_point_clouds[t][obj_["int_id"]], axis=0),axis=0)]
                                    n_pts = overall_point_clouds[t][obj_["int_id"]][0].shape[0]
                                    tsk_group.attrs["max_num_points"] = max(tsk_group.attrs["max_num_points"], n_pts)
                                    obj_pt_cl = frame_group.create_dataset(str(i), (n_pts, pt_low_dim), dtype='f')
                                    obj_pt_cl[...] = overall_point_clouds[t][obj_["int_id"]]
                                    ### define max_nb points in h5py file attributes ##


                                    """if "target" in res_.keys():
                                        overall_point_cloud.append(res_["target"]["3d_points"])
                                        overall_point_cloud = [np.unique(np.concatenate(overall_point_cloud, axis=0), axis=0)]
                                    if "distractor0" in res_.keys():
                                        overall_point_cloud.append(res_["distractor0"]["3d_points"])
                                        overall_point_cloud = [np.unique(np.concatenate(overall_point_cloud, axis=0), axis=0)]
                                    if "distractor1" in res_.keys():
                                        overall_point_cloud.append(res_["distractor1"]["3d_points"])
                                        overall_point_cloud = [np.unique(np.concatenate(overall_point_cloud, axis=0), axis=0)]"""
                            ...
                            #input()
                        """colors_points = ['red', 'green', 'blue', 'black', 'orange']#np.arange(5)/5
                        fig = plt.figure()
                        ...
                        for i, (k, obj_) in enumerate(overall_point_clouds[0].items()):
                            print()
                            X = obj_[0][:, 0]
                            Y = obj_[0][:, 1]
                            Z = obj_[0][:, 2]
                            ...
                            all_Y.append(Y)
                            all_Z.append(Z)
                            ax.scatter(X, Y, Z, color=colors_points[i])
                        ...
                        plt.show()
                        print(len(obj_tab["left_shoulder"]))
                        input()
                        #print(keypoints)"""
                        n_seen_trajs += 1
...
def count_points_per_frame(path_dataset, config, use_3D_gt=True, action_range="next_ts", visited_states= 'all', goal=None, keep_gripper_close_states=False):
    visited_states_ = visited_states
    # if task!=None:
    object_names_to_id = {}
    n_obj_per_img = 0
    ...
    for mode in ['train']:
        ...
        if len(tasks) > 0:
            ...
            for tsk in tasks:
                ...
                n_obj_per_img = len(colors)
                print('task ' + str(tsk))
                os.mkdir(path_dataset + mode + "/" + str(tsk) + "/pt_clouds/clouds_sizes/")
                max_pts_in_whole_traj = 0
                max_pts_in_first_frame = 0
                for var in range(config["n_variation_per_task"]):
                    print('var ' + str(var))
                    for ex in range(config["n_ex_per_variation"]):
                        print('ex ' + str(ex))
                        keypoints['S1']['task_'+str(tsk)+'_exemple_'+str(ex)]=[]
                        #os.mkdir(path_dataset + mode + "/" + str(tsk) + "/pt_clouds/clouds_sizes/"+str(ex)+"/")
                        #traj_grp = h5py_dataset["data"].create_group("demo_" + str(n_seen_trajs))
                        if mode == "train":
                            train_ids.append("demo_" + str(n_seen_trajs))
                        elif mode == "test":
                        ...
                        for cam in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                            ...
                            #input()
                        points_in_ex = []
                        for i_t, (k_t, v_t) in enumerate(overall_point_clouds.items()):
                            points_in_frame = 0
                            for i, (k, obj_) in enumerate(v_t.items()):
                                points_in_frame+=obj_.shape[0]
                            points_in_ex.append(points_in_frame)
                            if points_in_frame>max_pts_in_whole_traj:
                                max_pts_in_whole_traj = points_in_frame

                        np.save(path_dataset + mode + "/" + str(tsk) + "/pt_clouds/clouds_sizes/" + str(ex) + ".npy", np.array(points_in_ex))

                        if points_in_ex[0]>max_pts_in_first_frame:
                            max_pts_in_first_frame = points_in_ex[0]
                ...
                f = open(path_dataset + mode + "/" + str(tsk) + "/pt_clouds/clouds_sizes/overall_n_pts.txt", 'w')
                f.write(max_pts_in_whole_traj)
                f.close()

                f = open(path_dataset + mode + "/" + str(tsk) + "/pt_clouds/clouds_sizes/first_frame_n_pts.txt", 'w')
                f.write(max_pts_in_first_frame)
                f.close()
...
def export_dataset_pt_cloud(path_dataset, config, use_3D_gt=True, action_range="next_ts", visited_states= 'all', goal=None, keep_gripper_close_states=False):
    ...
    for mode in ['train']:
        ...
        if len(tasks) > 0:
            ...
            for tsk in tasks:
                data = []
                pt_cl_dataset = h5py.File(str(path_dataset) + 'ply_data_train_task' + str(tsk) + '.h5', 'w')
                data = pt_cl_dataset.create_dataset('data', (300, n_scene_pts, 4))
                labels = pt_cl_dataset.create_dataset('label', (300, n_waypts, 8))
