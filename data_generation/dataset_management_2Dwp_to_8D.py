...
def export_dataset_to_keypoints_estimation_data_1_frame(path_dataset, config, use_3D_gt=True, action_range="next_ts", visited_states= 'all', goal=None, keep_gripper_close_states=False, mode="train"):
    visited_states_ = visited_states
    # if task!=None:
    object_names_to_id = {}
    n_obj_per_img = 0
                ...
                for var in range(config["n_variation_per_task"]):
                    ...
                    for ex in range(config["n_ex_per_variation"]):
                        ...
                        for cam in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                            ...
                            #for t in range(seq_len):
                            with Pool(initializer=init_worker, initargs=(visited_states_, tsk, cam, ex, var, None, None, obj_poses, traj_obj_poses, objects_strId_to_intId, mode, cam_transf, ee_states, wp_hist, keypoints,)) as pool:
                                # call the same function with different data in parallel
...
def export_dataset_to_keypoints_estimation_data(path_dataset, config, use_3D_gt=True, action_range="next_ts", visited_states= 'all', goal=None, keep_gripper_close_states=False):
    visited_states_ = visited_states
    # if task!=None:
    object_names_to_id = {}
    n_obj_per_img = 0
...
if __name__ == "__main__":
    ...
    if operation==1:
        ...
        mode="train"
        if args.one_frame:
            export_dataset_to_keypoints_estimation_data_1_frame(path_dataset, config, action_range=args.action_range, visited_states='all', mode=mode)
        else:
