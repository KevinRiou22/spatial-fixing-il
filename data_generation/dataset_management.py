...
def get_all_objects_names(path_dataset, config, mode="train"):
    ...
    for task_i in sim_tasks:
        try:
            ...
            stat_file_id = "/" + str(config[mode+"_task_ids"][i])
            filename = path_dataset + mode + str(stat_file_id) + "/all_RLBench_objects_names.txt"
...
def get_cameras_matrixes(config, mode):
    obs_config = ObservationConfig(image_size=(config["obs_dim"][0], config["obs_dim"][1]), record_gripper_closing=True)
    obs_config.set_all(True)
    # obs_config.set_all_high_dim(False)
    # obs_config.set_all_low_dim(True)
                ...
                for d in range(self.config['n_ex_per_variation']):
                    ...
                    while (not demo_collect_sucess) and trials < self.max_traj_trials:
                        try:
                            ...
                            print("demo collect fail "+str(trials))
                            trials += 1
...
def process_one_ts_keypoints(t):
    ...
    for i, (kp_id, kp) in enumerate(frame_keypoints.items()):
        ...
        """if kp["parent_id"] not in visible_objects:
            ...
            uncertainty = 0
            visibility = 1"""
        uncertainty = 3*np.random.normal(loc=0.0, scale=0.00504, size=(2, ))
        #print("2x uncertainty process_one_ts")
        ...
        image = cv2.circle(image, (int(pt_2d_gt[0])*2, int(pt_2d_gt[1])*2), radius=2, color = (255, 255, 255), thickness=-1)
        image = cv2.circle(image, (int(pt_2d_pred[0]*2), int(pt_2d_pred[1])*2), radius=2, color=(0, 0, 0), thickness=-1)
...
def export_dataset_to_pose_estimation_data(path_dataset, config, use_3D_gt=True, action_range="next_ts", visited_states= 'all', goal=None, keep_gripper_close_states=False):
    ...
    out_filename_base = out_filename_base+'_3x_noise'
    ...
    np.savez(out_filename_base + '_images.npz', **dataset_imgs)
    #return h5py_dataset, total_samples, train_ids, valid_ids


def export_dataset_to_keypoints_estimation_data(path_dataset, config, use_3D_gt=True, action_range="next_ts", visited_states= 'all', goal=None, keep_gripper_close_states=False, mode="train"):
    visited_states_ = visited_states
    # if task!=None:
    object_names_to_id = {}
    n_obj_per_img = 0
    for task in config[mode+"_task_ids"]:
        object_names_to_id = load_objects_names(
            path_dataset + mode+"/" + str(task) + "/all_RLBench_objects_names_cleaned.txt", old_dict=object_names_to_id)
        print(object_names_to_id)
        file_obj_poses = open(path_dataset + "train/" + str(task) + "/nb_ob_per_img.json", 'r')
        obj_poses = json.load(file_obj_poses)
        #n_obj_per_img = max(obj_poses["nb_ob_per_img_max"], n_obj_per_img)
    ...
    # low_dim = (6 + n_obj_classes)
    if config['use_object_quaternions']:
        low_dim = 1+2+7+1 # obj_id + 2d pose + 3D pose&quat + visibility
    else:
        #low_dim = 1+2+3+1 # obj_id + 2d pose + 3D pose + visibility
        low_dim =  2 + 2+ 3 + 1  #  2d pose GT + 2D pose 'pred' + 3D pose + visibility
    low_dim = 8
    n_seen_trajs = 0
    total_samples = 0
    train_ids, valid_ids = [], []
    ...
    out_filename_base = out_filename_base+'_with_uncertainty'
    for mode in [mode]:
        if mode == 'train':
            tasks = config["train_task_ids"]
        elif mode == 'test':
...
def get_dataset_stats(path_dataset, config, mode):
    all_actions_mins = []
    all_actions_maxs = []
    all_ee_mins = []
    all_ee_maxs = []
    ...
    train_tasks = FS95_V1['train']
    test_tasks = FS95_V1['test']
    for tsk in config[mode+"_task_ids"]:
        all_tsk_lens = []
        all_tsk_wp_lens = []
        for var in range(config["n_variation_per_task"]):
            for ex in range(config["n_ex_per_variation"]):
                try:
                    hist_action_path = path_dataset + mode +"/" + str(tsk) + "/actions/gripper_pose/" + str(var) + "/" + str(
                        ex) + ".npy"
                    hist_ee_path = path_dataset + mode +"/" + str(tsk) + "/ee_state/" + str(var) + "/" + str(ex) + ".npy"
                    wp_path = path_dataset + mode +"/" + str(tsk) + "/waypoints/" + str(var) + "/actual_waypoints_ts_" + str(ex) + ".npy"
                    obj_poses_path = path_dataset + mode +"/" + str(tsk) + "/obj_poses/" + str(var) + "/" + str(ex) + ".json"
                    #print("got all paths")
                    #print(hist_action_path)
                    ...
                    ee_states = np.load(hist_ee_path)
                    #print("ee_states.shape : " + str(ee_states.shape))
                    #input()
                    file_obj_poses = open(obj_poses_path, 'r')
                    obj_poses = json.load(file_obj_poses)
                    n_objs = []
        ...
        try:
            ...
            np.save(path_dataset + mode+str(stat_file_id)+"/action_mins.npy", all_actions_mins)
            np.save(path_dataset + mode+str(stat_file_id)+ "/action_maxs.npy", all_actions_maxs)
            np.save(path_dataset + mode+str(stat_file_id)+ "/ees_mins.npy", all_ee_mins)
            np.save(path_dataset + mode+str(stat_file_id)+ "/ees_maxs.npy", all_ee_maxs)
            np.save(path_dataset + mode + str(stat_file_id) + "/obj_pose_mins.npy", all_obj_pose_mins)
            np.save(path_dataset + mode + str(stat_file_id) + "/obj_pose_maxs.npy", all_obj_pose_maxs)
            with open(path_dataset + mode+str(stat_file_id)+ "/nb_ob_per_img.json", 'w') as fp:
        ...
        if mode =="train":
            ...
            tsk_name = vars(test_tasks[tsk])['__module__'].split('.')[-1]
            print(tsk_name)
        try:
...
if __name__ == "__main__":
    ...
    if operation==1:
        ...
        export_dataset_to_keypoints_estimation_data(path_dataset, config, action_range=args.action_range, visited_states='all')
        ...
        get_dataset_stats(path_dataset, config, mode)
        filenames = get_all_objects_names(path_dataset, config, mode=mode)
        ...
        if config['train_task_ids'][0] == "*":
            ...
            if mode == 'train':
                ...
                n_tasks = len(test_tasks)
                config['test_task_ids'] = np.arange(n_tasks).tolist()
        get_cameras_matrixes(config, mode)
    elif operation==5:
