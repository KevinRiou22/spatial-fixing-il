    ...
    for mode in [mode]:
        ...
        if len(tasks) > 0:
            ...
            for tsk in tasks:
                ...
                np.savez("../model/data/RLBench_pose_est_data/" + out_filename_base + '_task_' + str(tsk) + '.npz', **dataset)
                np.savez("../model/data/RLBench_pose_est_data/" + out_filename_base + '_images_task_' + str(tsk) + '.npz', **dataset_imgs)
    np.savez("../model/data/RLBench_pose_est_data/" + out_filename_base + '.npz', **dataset)
    np.savez("../model/data/RLBench_pose_est_data/" + out_filename_base + '_images.npz', **dataset_imgs)
    # return h5py_dataset, total_samples, train_ids, valid_ids
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action_range', type=str,
                        help='next_ts --> actions are next timestep pose, next_wp --> actions are next waypoint pose',
                        required=False)
    parser.add_argument('--action_on_data', type=str,
                        help='choose among {"get_data_stats", "convert_data", "get_cameras_matrixes", "visualize_segmentation_colors"}',
                        required=False)
    ...
    if args.action_on_data=="convert_data":
    ...
    elif args.action_on_data == "get_data_stats":
        if config['train_task_ids'][0] == "*":
            mode = "train"
            train_tasks = FS95_V1['train']
            test_tasks = FS95_V1['test']
    ...
    elif args.action_on_data == "get_cameras_matrixes":
        if config['train_task_ids'][0] == "*":
            mode = "train"
            train_tasks = FS95_V1['train']
            test_tasks = FS95_V1['test']
    ...
    elif args.action_on_data == "visualize_segmentation_colors":
