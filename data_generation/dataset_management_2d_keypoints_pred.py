                ...
                for var in range(config["n_variation_per_task"]):
                    ...
                    for ex in range(config["n_ex_per_variation"]):
                        ...
                        for cam in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                            ...
                            traj_obj_poses = np.zeros((1, n_obj_per_img, low_dim))
                            ...
                            for t in range(1):
                                ...
                                if visualize_data:
                                    ...
                                    for i, (k, object_bbx) in enumerate(boxes.items()):
                                        ...
                                        #traj_obj_poses[t, n_obj * low_dim:(n_obj + 1) * low_dim] = normed_pose
                                    for wp_i in range(waypoints.shape[0]):
                                        pt_2d = np.matmul(cam_transf[cam], np.concatenate([np.array(waypoints[wp_i, :3]), [1]]))
                                        pt_2d[:2] = (pt_2d[:2] / pt_2d[2])
                                        img = cv2.circle(img, (int(pt_2d[0])*2, int(pt_2d[1])*2), radius=2, color=color, thickness=-1)
                                    if t==0:
                                        plt.imshow(img, cmap='gray')
                                        plt.figure()
                                        plt.imshow(numpymask)
                                ...
                                for obj in obj_poses[str(ts)]:
                                    ...
                                    if obj["id"] in boxes.keys():
                                        ...
                                        detect_2d_gt = [boxes[obj["id"]]["min_w"], boxes[obj["id"]]["max_w"],
                                                        boxes[obj["id"]]["min_h"],
                                                        boxes[obj["id"]]["max_h"], boxes[obj["id"]]["min_z"]["depth"],
                                                        boxes[obj["id"]]["max_z"]["depth"]]
                                        # obj_tab.append(np.concatenate([np.array([objects_strId_to_intId[obj["id"]]]), np.array(detect_2d_gt)], axis=-1))
                                        traj_obj_poses[t, n_obj] = np.concatenate([np.array([objects_strId_to_intId[obj["id"]]])/len(objects_strId_to_intId), np.array(detect_2d_gt)/255],
                                            axis=-1)
                                        #print("traj_obj_poses[t, n_obj] : "+str(traj_obj_poses[t, n_obj]))
                                        # print("t "+str(t) + " : "+str(traj_obj_poses[t, n_obj]))
                                        # input()
                                        n_obj += 1
                            #input()
