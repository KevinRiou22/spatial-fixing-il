...
def get_cameras_matrixes(config):
    ...
    for cam in cams:
        np.save('cameras_matrixes/' + cam + '_intrinsics.npy',
                np.array(eval("task._scene." + cam + ".get_intrinsic_matrix()")))
...
class Waypoints_demo_generator():
    ...
    def get_demos(self, task_ids):
        fps = 10
        obs_config = ObservationConfig(image_size=(self.config["obs_dim"][0], self.config["obs_dim"][1]),
        ...
        for task_i in sim_tasks:
            ...
            for ex in range(self.config['n_variation_per_task']):
                ...
                os.mkdir(dir_task + "/seeds/" + str(ex))
                ...
                for d in range(self.config['n_ex_per_variation']):
                    ...
                    while (not demo_collect_sucess) and trials < self.max_traj_trials:
                        try:
                            raw_demos, waypoints_ts = task.get_demos(1,
                                                                     live_demos=live_demos)  # n_demos + current trajectory
                            demo_collect_sucess = True
                        except:
                            trials += 1
                    if not demo_collect_sucess:
                        fail_logs.write(
                            "task " + str(task_i) + ", task_idx " + str(task_idx) + ", var " + str(ex) + ", ex " + str(
                                d) + str("\n"))
                    ...
                    print(waypoints_ts)
                    print(raw_demos)
                    current_seeds = raw_demos[0].random_seed
                    seeds_f = open(dir_task + "/seeds/" + str(ex) + "/" + str(d) + ".txt", 'a')
                    seeds_f.write(
                        str(current_seeds[0]) + "," + str(current_seeds[2]) + "," + str(current_seeds[3]) + "," + str(
                            current_seeds[4]) + "\n")
                    seeds_f.close()
                    filename = dir_task + "/seeds/" + str(ex) + "/" + str(d)
                    ...
                    if len(cameras) > 0:
                        for cam in cameras:
                            ...
                            if self.config['get_segmentations']:
                                save_vid_path_seg = dir_task + "/obs_segmentations/" + cam + "/" + str(ex) + "/" + str(
                                    d) + ".mp4"
                            ...
                            for t in range(dem_len):
                                ...
                                np.save(dir_task + "/pt_clouds/" + cam + "/" + str(ex) + "/" + str(d) + "_" + str(
                                    t) + ".npy", pt_cloud)
                                # data[cam][t, :, :, :] = np.concatenate([rgb, depth], axis=2)
                                # out_rgb.write(rgb)
                                # out_depth.write(depth)
                                cv2.imwrite(
                                    dir_task + "/obs/" + cam + "/" + str(ex) + "/" + str(d) + "_rgb_" + str(t) + ".png",
                                    rgb)
                                cv2.imwrite(dir_task + "/obs/" + cam + "/" + str(ex) + "/" + str(d) + "_depth_" + str(
                                    t) + ".png", depth)
                                if self.config['get_segmentations']:
                                    cam_seg = cam + "_mask"
                                    seg = np.array(eval('raw_demos[0][t].' + str(cam_seg))).astype(np.uint8)
                                    # out_seg.write(seg)
                                    cv2.imwrite(dir_task + "/obs_segmentations/" + cam + "/" + str(ex) + "/" + str(
                                        d) + "_" + str(t) + ".png", seg)
                                # print(depth)
                            # out_rgb.release()
                            # out_depth.release()
                    ...
                    np.save(dir_task + "/waypoints/" + str(ex) + "/actual_waypoints_ts_" + str(d) + ".npy",
                            np.array(waypoints_ts))
                    for registered_action in registered_actions:
...
def get_trajectories(path_dataset, config, h5py_dataset, action_range="next_ts", visited_states='all', goal=None,
                     keep_gripper_close_states=False):
    # if task!=None:
    object_names_to_id = {}
    n_obj_per_img = 0
            ...
            for tsk in tasks:
                ...
                for var in range(config["n_variation_per_task"]):
                    for ex in range(config["n_ex_per_variation"]):
                        ...
                        obj_poses = json.load(file_obj_poses)
                        file_wp_hist = path_dataset + mode + "/" + str(tsk) + "/waypoints/" + str(
                            var) + "/actual_waypoints_ts_" + str(ex) + ".npy"
                        ...
                        if visited_states == 'wp_only':
                            ...
                            actions_data = traj_grp.create_dataset("actions", (seq_len,) + ee_states[0, :].shape,
                                                                   dtype='f')
                        else:
                        ...
                        for t in range(seq_len):
                            ...
                            for obj in obj_poses[str(ts)]:
                                normed_pose = (np.array(obj[
                                                            "bbox"]) - obj_pose_mins) / obj_pose_ranges  # 2 * normalize_bbox(np.array(obj["bbox"])) - 1
                                ...
                                concat_obj_infos = np.concatenate(
                                    [np.array([float(object_names_to_id[obj["name"]]) / n_obj_classes]), normed_pose],
                                    axis=-1)
...
def init_worker(visited_states_, tsk, cam, ex, var, colors, mask_colors_to_obj_names, obj_poses, traj_obj_poses,
                objects_strId_to_intId, mode, cam_transf, ee_states=None, wp_hist=None, keypoints=None):
...
def process_one_ts_data_to_pose_from_image(t):
    ...
    for obj in obj_poses[str(ts)]:
        ...
        if obj["id"] in boxes.keys():
            ...
            traj_obj_poses[t, n_obj] = np.concatenate(
                [np.array([objects_strId_to_intId[obj["id"]]]), np.array(detect_2d_gt), normed_pose], axis=-1)
...
def process_one_ts_data_to_pose_from_3D(t):
    ...
    for obj in _obj_poses[str(ts)]:
        ...
        if obj["id"] in visible_objects:
            ...
            obj_tab.append(
                np.concatenate([np.array([_objects_strId_to_intId[obj["id"]]]), np.array(box_2d), np.array(box_3d)],
                               axis=-1))
...
def process_one_ts_data_to_pose_from_3D_full_pose(t):
    ...
    for obj in _obj_poses[str(ts)]:
        ...
        quat = np.array(obj["quat"])
        quat = np.concatenate(
            [np.array([quat[-1]]), quat[:-1]])  # qw is last from pyrep, but first in "common" representations
        ...
        if obj["id"] in visible_objects:
            ...
            # _traj_obj_poses[t, n_obj] = np.concatenate([np.array([_objects_strId_to_intId[obj["id"]]]), np.array(box_2d), normed_pose], axis=-1)
            # obj_tab.append(np.concatenate([np.array([_objects_strId_to_intId[obj["id"]]]), np.array(box_2d), normed_pose], axis=-1))
            obj_tab.append(np.concatenate(
                [np.array([_objects_strId_to_intId[obj["id"]]]), np.array(box_2d), obj_center, obj_dims, quat],
                axis=-1))
            # print(_traj_obj_poses[t, n_obj])
            """print(box_2d)
            if t==0:
...
def process_one_ts_keypoints(t):
    ...
    # ts = wp_hist[t] if _visited_states_ == 'wp_only' else t
    concat_obj_infos = np.array([])
    np_path = path_dataset + _mode + "/" + str(_tsk) + "/obs_segmentations/" + str(_cam) + "/" + str(_var) + "/" + str(
        _ex) + "_" + str(t) + ".png"
    numpymask = np.mean(cv2.imread(np_path), axis=-1).astype(np.uint8)

    image_path = path_dataset + _mode + "/" + str(_tsk) + "/obs/" + str(_cam) + "/" + str(_var) + "/" + str(
        _ex) + "_" + str(t) + ".png"
    ...
    for i, (kp_id, kp) in enumerate(frame_keypoints.items()):
        ...
        # obj_id = np.array([_objects_strId_to_intId[kp["parent_id"]]])
        # print("parent name : {}, parent id : {}".format(kp["parent_id"], obj_id))
        pt_2d = np.matmul(_cam_transf[_cam], np.concatenate([np.array(gt_3D[:3]), [1]]))
        ...
        image = cv2.circle(image, (int(pt_2d_gt[0]) * 2, int(pt_2d_gt[1]) * 2), radius=2, color=(255, 255, 255),
                           thickness=-1)
        image = cv2.circle(image, (int(pt_2d_pred[0] * 2), int(pt_2d_pred[1]) * 2), radius=2, color=(0, 0, 0),
                           thickness=-1)
        # obj_tab.append(np.concatenate( [obj_id, inp_2D, gt_3D, np.array([visibility])], axis=-1))
        obj_tab.append(np.concatenate([pt_2d_gt, pt_2d_pred, gt_3D, np.array([visibility])], axis=-1))
    # print("ts {}, cam {} : {}".format(t, _cam, obj_tab))
    cv2.imwrite("/media/facebooksurrounduser/data/deb/ex{}_cam{}_t{}.jpg".format(_ex, _cam, t), image)
    return (t, obj_tab)


def export_dataset_to_pose_estimation_data(path_dataset, config, use_3D_gt=True, action_range="next_ts",
                                           visited_states='all', goal=None, keep_gripper_close_states=False):
                ...
                for var in range(config["n_variation_per_task"]):
                    ...
                    for ex in range(config["n_ex_per_variation"]):
                        ...
                        for cam in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                            ...
                            visited_states_, tsk, cam, ex, var, colors, mask_colors_to_obj_names, obj_poses,
                            traj_obj_poses, objects_strId_to_intId, mode, cam_transf,)) as pool:
...
def export_dataset_to_keypoints_estimation_data_1_frame(path_dataset, config, use_3D_gt=True, action_range="next_ts",
                                                        visited_states='all', goal=None,
                                                        keep_gripper_close_states=False, mode="train"):
    visited_states_ = visited_states
    # if task!=None:
    object_names_to_id = {}
    n_obj_per_img = 0
    for task in config[mode + "_task_ids"]:
        object_names_to_id = load_objects_names(
            path_dataset + mode + "/" + str(task) + "/all_RLBench_objects_names_cleaned.txt",
            old_dict=object_names_to_id)
        print(object_names_to_id)
        file_obj_poses = open(path_dataset + mode + "/" + str(task) + "/nb_ob_per_img.json", 'r')
        obj_poses = json.load(file_obj_poses)
        # n_obj_per_img = max(obj_poses["nb_ob_per_img_max"], n_obj_per_img)
    # else:
    #     object_names_to_id = load_objects_names("./all_RLBench_objects_names_cleaned.txt")
    ...
    # n_obj_classes = len(object_names_to_id) + 1  # +1 for gripper
    # low_dim = (6 + n_obj_classes)
    low_dim = 2 + 2 + 8 + 1  # 2d pose GT + 2D pose 'pred' + 3D pose + visibility
    n_seen_trajs = 0
    total_samples = 0
    train_ids, valid_ids = [], []
    dataset = {'S1': {}}
    ...
    out_filename_base = "2d_wp_to_8d_wp_1_frame"
    ...
    for mode in [mode]:
        ...
        if len(tasks) > 0:
            ...
            # print(objects_strId_to_intId)
            for tsk in tasks:
                ee_maxs = np.load(path_dataset + mode + "/" + str(tsk) + "/ees_maxs.npy")
                ee_mins = np.load(path_dataset + mode + "/" + str(tsk) + "/ees_mins.npy")
                obj_pose_maxs = np.load(path_dataset + mode + "/" + str(tsk) + "/obj_pose_maxs.npy")
                obj_pose_mins = np.load(path_dataset + mode + "/" + str(tsk) + "/obj_pose_mins.npy")
                ee_ranges = ee_maxs - ee_mins
                obj_pose_ranges = obj_pose_maxs - obj_pose_mins
                # mask_colors_to_obj_names = json.load(open('./obj_masks_colors/'+str(tsk)+'.json', 'r'))
                # colors = mask_colors_to_obj_names.keys()
                ...
                for var in range(config["n_variation_per_task"]):
                    ...
                    for ex in range(config["n_ex_per_variation"]):
                        ...
                        dataset_imgs['S1']['task_' + str(tsk) + '_exemple_' + str(ex)] = []
                        # traj_grp = h5py_dataset["data"].create_group("demo_" + str(n_seen_trajs))
                        if mode == "train":
                            train_ids.append("demo_" + str(n_seen_trajs))
                        elif mode == "test":
                        ...
                        obj_poses_path = path_dataset + mode + "/" + str(tsk) + "/obj_poses/" + str(var) + "/" + str(
                            ex) + ".json"
                        keypoints_path = path_dataset + mode + "/" + str(tsk) + "/traj_keypoints/" + str(
                            var) + "/" + str(
                            ex) + ".json"
                        ...
                        for cam in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                            ...
                            traj_obj_poses = np.zeros((1, len(keypoints[str(0)].keys()), low_dim))

                            images_idxs = np.zeros((1, 4))
                            # for t in range(seq_len):
                            with Pool(initializer=init_worker, initargs=(
                            visited_states_, tsk, cam, ex, var, None, None, obj_poses, traj_obj_poses,
                            objects_strId_to_intId, mode, cam_transf, ee_states, wp_hist, keypoints,)) as pool:
                                # call the same function with different data in parallel
                                res = pool.map(process_one_ts_keypoints, range(1))
                            t = 0
                            for res_ in res:
                                if len(res_[1]) > 0:
                                    res_objs = np.array(res_[1])
                                    ...
                                    traj_obj_poses[res_[0], :res_objs.shape[0]] = res_objs
                                images_idxs[t] = np.array([tsk, var, ex, t])
                                t += 1
                            # input()
                            # print(traj_obj_poses)
                            # print("ts {}, cam {} : {}".format(0, cam, traj_obj_poses[0]))
                            # input()
                            dataset['S1']['task_' + str(tsk) + '_exemple_' + str(ex)].append(traj_obj_poses)
                            dataset_imgs['S1']['task_' + str(tsk) + '_exemple_' + str(ex)].append(images_idxs)
                            # print(keypoints['S1']['task_'+str(tsk)+'_exemple_'+str(ex)])
                            # input()
                        # print(keypoints)
                        n_seen_trajs += 1

                np.savez(path_dataset + out_filename_base + '_task_' + str(tsk) + '.npz', **dataset)
                np.savez(path_dataset + out_filename_base + '_images_task_' + str(tsk) + '.npz', **dataset_imgs)
    np.savez(path_dataset + out_filename_base + '.npz', **dataset)
    np.savez(path_dataset + out_filename_base + '_images.npz', **dataset_imgs)
    # return h5py_dataset, total_samples, train_ids, valid_ids


def export_dataset_to_keypoints_estimation_data(path_dataset, config, use_3D_gt=True, action_range="next_ts",
                                                visited_states='all', goal=None, keep_gripper_close_states=False):
    visited_states_ = visited_states
    # if task!=None:
    object_names_to_id = {}
    ...
    out_filename_base = "2d_wp_to_8d_wp"
            ...
            for tsk in tasks:
                ...
                for var in range(config["n_variation_per_task"]):
                    ...
                    for ex in range(config["n_ex_per_variation"]):
                        ...
                        for cam in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                            # obj_data = obs_grp.create_dataset("object", (seq_len, n_obj_per_img * low_dim), dtype='f')
                            traj_obj_poses = np.zeros((seq_len, len(keypoints[str(0)].keys()), low_dim))

                            images_idxs = np.zeros((seq_len, 4))
                            # for t in range(seq_len):
                            with Pool(initializer=init_worker, initargs=(
                            visited_states_, tsk, cam, ex, var, colors, mask_colors_to_obj_names, obj_poses,
                            traj_obj_poses, objects_strId_to_intId, mode, cam_transf, ee_states, wp_hist,
                            keypoints,)) as pool:
                                # call the same function with different data in parallel
                ...
                np.savez(path_dataset + out_filename_base + '_task_' + str(tsk) + '.npz', **dataset)
                np.savez(out_filename_base + '_images_task_' + str(tsk) + '.npz', **dataset_imgs)
    np.savez(path_dataset + out_filename_base + '.npz', **dataset)
    np.savez(path_dataset + out_filename_base + '_images.npz', **dataset_imgs)
    # return h5py_dataset, total_samples, train_ids, valid_ids
...
def visualize_mask_colors(path_dataset, config, tasks=None, action_range="next_ts", visited_states='all', goal=None,
                          keep_gripper_close_states=False):
    object_names_to_id = {}
    n_obj_per_img = 0
    for task in config["train_task_ids"]:
                        ...
                        for cam in ["left_shoulder", "right_shoulder", "overhead", "front"]:
                            ...
                            for t in range(seq_len):
                                ...
                                for obj in obj_poses[str(ts)]:
                                    ...
                                    obj_center = np.array(
                                        [(normed_pose[1] + normed_pose[0]) / 2, (normed_pose[3] + normed_pose[2]) / 2,
                                         (normed_pose[5] + normed_pose[4]) / 2])
                                    for x_i in [0, 1]:
                                        for y_i in [2, 3]:
                                            for z_i in [4, 5]:
                                                ...
                                                color = (255, 0, 0)
                                                # if obj["id"]=='usb':
                                                img = cv2.circle(img, (int(pt[0]), int(pt[1])), radius=2, color=color,
                                                                 thickness=-1)
...
def get_trajectories_separate_objs(path_dataset, config, h5py_dataset, action_range="next_ts", visited_states='all',
                                   goal=None):
    # if task!=None:
    object_names_to_id = {}
    n_obj_per_img = 0
                ...
                for var in range(config["n_variation_per_task"]):
                    for ex in range(config["n_ex_per_variation"]):
                        ...
                        for obj in obj_poses[str(0)]:
                            obj_datas.append(
                                obs_grp.create_dataset("object_" + str(obj_i), (seq_len, low_dim), dtype='f'))
                        ...
                        obj_i = 0
                        for obj in obj_poses[str(0)]:
                            obj_datas_next.append(
                                next_obs_grp.create_dataset("object_" + str(obj_i), (seq_len, low_dim), dtype='f'))
                            traj_obj_poses = np.zeros((seq_len, low_dim))
                            for t in range(1, seq_len + 1):
                                n_obj = 0
                                concat_obj_infos = np.array([])
...
def export_dataset_to_robomimic_format(dataset_path, config, action_range="next_ts", visited_states='all', goal=None,
                                       robomimic_path='low_dim', robomimic_name='low_dim', task=None):
    f = h5py.File(str(robomimic_path) + '.hdf5', 'w')
    grp = f.create_group("data")
    grp.attrs["env_args"] = json.dumps(env_meta_data(config, action_range, robomimic_name))
    f, total_samples, train_ids, valid_ids = get_trajectories(dataset_path, config, f, action_range=action_range,
                                                              visited_states=visited_states, goal=goal)
...
def get_dataset_stats(path_dataset, config):
    ...
    for tsk in config["train_task_ids"]:
        ...
        for var in range(config["n_variation_per_task"]):
            for ex in range(config["n_ex_per_variation"]):
                try:
                    hist_action_path = path_dataset + "train/" + str(tsk) + "/actions/gripper_pose/" + str(
                        var) + "/" + str(
                        ex) + ".npy"
                    hist_ee_path = path_dataset + "train/" + str(tsk) + "/ee_state/" + str(var) + "/" + str(ex) + ".npy"
                    wp_path = path_dataset + "train/" + str(tsk) + "/waypoints/" + str(
                        var) + "/actual_waypoints_ts_" + str(ex) + ".npy"
                    obj_poses_path = path_dataset + "train/" + str(tsk) + "/obj_poses/" + str(var) + "/" + str(
                        ex) + ".json"
        ...
        try:
            ...
            with open(path_dataset + "train" + str(stat_file_id) + "/nb_ob_per_img.json", 'w') as fp:
                json.dump({"nb_ob_per_img_min": int(nb_ob_per_img_min), "nb_ob_per_img_max": int(nb_ob_per_img_max)},
                          fp)
        except:
        ...
        print(tsk_name)
        try:
            tasks_lens[str(tsk)] = {'name': tsk_name, 'mean_len': float(np.mean(all_tsk_lens)),
                                    'max_len': float(np.max(all_tsk_lens)), 'min_len': float(np.min(all_tsk_lens)),
                                    'std_len': float(np.std(all_tsk_lens)), 'n_ex': int(all_tsk_lens.shape[0])}
            tasks_wp_lens[str(tsk)] = {'name': tsk_name, 'mean_len': float(np.mean(all_tsk_wp_lens)),
                                       'max_len': float(np.max(all_tsk_wp_lens)),
                                       'min_len': float(np.min(all_tsk_wp_lens)),
                                       'std_len': float(np.std(all_tsk_wp_lens)), 'n_ex': int(all_tsk_wp_lens.shape[0])}

        except:
            tasks_lens[str(tsk)] = {'name': tsk_name, 'mean_len': -1, 'max_len': -1, 'min_len': -1, 'std_len': -1,
                                    'n_ex': 0}
            tasks_wp_lens[str(tsk)] = {'name': tsk_name, 'mean_len': -1, 'max_len': -1, 'min_len': -1, 'std_len': -1,
                                       'n_ex': 0}
...
class Dataset():
    ...
    def load_traj(self, task, variation, exemple, ee_poses, actions, obs, all_waypoints, n_wps, batch_id, obj_poses,
                  demo_id=-1, training=True):
            ...
            for k_l in range(len(all_ts[0])):
                ...
                if ts >= 0:
                    ...
                    for obj in ob_pos[str(ts)]:
                        ...
                        concat_obj_infos = np.concatenate([indices_to_one_hot(int(self.object_names_to_id[obj["name"]]),
                                                                              self.n_obj_classes).flatten(),
                                                           normed_pose])
                        if demo_id == -1:
        ...
        ee_pos = ee_pos[
            all_ts]  # self.quat_to_euler_pose(ee_pos[all_ts])#self.normalize_ee_poses(self.quat_to_euler_pose(ee_pos[all_ts]))
        ee_pos[:, :, 0:3] = self.normalize_xyz_only(ee_pos[:, :, 0:3])
        act = act[
            all_glob_ts]  # self.quat_to_euler_pose(act[all_glob_ts])#self.normalize_actions(self.quat_to_euler_pose(act[all_glob_ts]))
    ...
    def load_batch_by_waypoints(self, demo_only=False):
        ...
        ee_poses_demos = np.zeros(
            (self.batch_size, self.n_demos, self.n_global_ts, self.n_local_ts, self.config["ee_state_dim"]))
        actions_demos = np.zeros((self.batch_size, self.n_demos, self.n_global_ts, self.config["ee_state_dim"]))
        obj_poses_demos = np.zeros(
            (self.batch_size, self.n_demos, self.n_global_ts, self.n_local_ts, self.n_obj, self.low_dim))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path_dataset_config', type=str, help='path to dataset config. ex: ./dataset_config.json',
                        required=True)
    parser.add_argument('--n_processes', type=int, help='', default=1)
    parser.add_argument('--skip_task_on_multi_fails', type=bool, help='', default=True)
    parser.add_argument('--max_traj_trials_bef_skip', type=int, help='', default=3)
    ...
    parser.add_argument('--one_frame', action='store_true', help='')
    ...
    operation = 1
    if operation == 1:
        n_processes = args.n_processes  # int(input("How much processes to parallelize on : "))
        skip_task_on_var_fail = args.skip_task_on_multi_fails  # bool(int(input("skip whole task on variation fail? No:0, Yes:1")))
        max_traj_trials = args.max_traj_trials_bef_skip  # int(input("number of trajectory trials before declaring var fail : "))
        try:
        ...
        if mode == 'train':
            if config['train_task_ids'][0] == "*":
                ...
                n_tasks = len(config['train_task_ids'])  # + len(config['test_task_ids'])
                print(n_tasks)
                tasks_per_processes = n_tasks // n_processes
                ...
                asssigned_tasks = [act_tasks[np.array(asssigned_tasks[i])] for i in range(
                    len(asssigned_tasks))]  # np.array(config['train_task_ids'])[np.array(asssigned_tasks)]
        else:
            if config['test_task_ids'][0] == "*":
        ...
        waypoints_demo_generator = Waypoints_demo_generator(path_dataset, config, mode=mode,
                                                            skip_task_on_var_fail=skip_task_on_var_fail,
                                                            max_traj_trials=max_traj_trials)
        print(asssigned_tasks)
        ...
        mode = "train"
        if args.one_frame:
            export_dataset_to_keypoints_estimation_data_1_frame(path_dataset, config, action_range=args.action_range,
                                                                visited_states='all', mode=mode)
        else:
            export_dataset_to_keypoints_estimation_data(path_dataset, config, action_range=args.action_range,
                                                        visited_states='all')
