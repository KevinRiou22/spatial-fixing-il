...
keypoints = dict(np.load(cfg.OBJ_POSE_DATA.ROOT_DIR+'/sim_all_formal_task_79_no_uncertainty.npz', allow_pickle=True))['pose'].item()
    ...
    for subject in subjects:
        ...
        for action in keypoints[subject].keys():
            ...
            if cfg.DATA.HAS_2D_PREDS:
                ...
                out_poses_2d_view1.append(poses_2d[0][:, :cfg.OBJ_POSE_DATA.NUM_JOINTS])
                out_poses_2d_view2.append(poses_2d[1][:, :cfg.OBJ_POSE_DATA.NUM_JOINTS])
                out_poses_2d_view3.append(poses_2d[2][:, :cfg.OBJ_POSE_DATA.NUM_JOINTS])
                out_poses_2d_view4.append(poses_2d[3][:, :cfg.OBJ_POSE_DATA.NUM_JOINTS])
...
if True:
    ...
    if cfg.TRAIN.RESUME:
        ...
        print('chk_filename : '+str(chk_filename))
        checkpoint = torch.load(chk_filename, map_location=lambda storage, loc: storage)
    ...
    min_full_pose_720_ex = torch.from_numpy(np.load("min_full_pose_720_ex.npy")).cuda()[:,:,:, :cfg.OBJ_POSE_DATA.NUM_JOINTS, :]
    max_full_pose_720_ex = torch.from_numpy(np.load("max_full_pose_720_ex.npy")).cuda()[:,:,:, :cfg.OBJ_POSE_DATA.NUM_JOINTS, :]
    range_full_pose_720_ex = max_full_pose_720_ex - min_full_pose_720_ex

    min_obj_center_cam_space_720_ex = torch.from_numpy(np.load("min_obj_center_cam_space_720_ex.npy")).cuda()[:, :, :,:cfg.OBJ_POSE_DATA.NUM_JOINTS, :]
    max_obj_center_cam_space_720_ex = torch.from_numpy(np.load("max_obj_center_cam_space_720_ex.npy")).cuda()[:, :, :,:cfg.OBJ_POSE_DATA.NUM_JOINTS, :]
    range_obj_center_cam_space_720_ex = max_obj_center_cam_space_720_ex - min_obj_center_cam_space_720_ex

    range_full_pose_720_ex_eval = range_full_pose_720_ex.permute((0, 1, 3, 4, 2)).contiguous()
    min_full_pose_720_ex_eval = min_full_pose_720_ex.permute((0, 1, 3, 4, 2)).contiguous()


    range_obj_center_cam_space_720_ex_eval = range_obj_center_cam_space_720_ex.permute((0, 1, 3, 4, 2)).contiguous()
    min_obj_center_cam_space_720_ex_eval = min_obj_center_cam_space_720_ex.permute((0, 1, 3, 4, 2)).contiguous()

    #print(range_obj_center_cam_space_720_ex_eval)
    #input()
    ...
    while epoch < cfg.TRAIN.NUM_EPOCHES:
        ...
        for batch_2d, sub_action in train_generator.next_epoch():
            ...
            if cfg.DATA.HAS_2D_PREDS:
                ...
                inputs_2d_gt[..., 0:cfg.NETWORK.INPUT_DIM, :] = inputs_2d_gt[..., 0:cfg.NETWORK.INPUT_DIM, :] / 256
            ...
            out = torch.mean(out, 2, keepdim=True).repeat(1, 1, 4, 1, 1)
            sup_loss = mpjpe(out, pos_gt[:, pad:pad + 1])
            # x = pos_gt[0, 0, 0, :, 0].detach().cpu().numpy()
            # y = pos_gt[0, 0, 0, :, 1].detach().cpu().numpy()
            # z = pos_gt[0, 0, 0, :, 2].detach().cpu().numpy()
            # fig = plt.figure("gt 3D")
            ...
            out_unsup = torch.cat((out, torch.ones(out.shape[:-1] + (1,)).cuda()), axis=-1)
            out_unsup = torch.permute(out_unsup.view(*out_unsup.shape[:3], -1, 4), (0, 1, 3, 2, 4)).contiguous()
            point_im_cam = torch.einsum('voi, btpvi->btpvo', full_transf, out_unsup)
            point_im_cam = torch.permute(point_im_cam, (0, 1, 3, 2, 4)).contiguous()
            point_im_cam = point_im_cam.view(*init_shape[:-1], 4)
            point_im_cam = torch.where(point_im_cam[:, :, :, :, 2:3]!=0, torch.divide(point_im_cam[:, :, :, :, :2], point_im_cam[:, :, :, :, 2:3]), 255.)
            # print("point_im_cam.shape : "+str(point_im_cam.shape))
            fig = plt.figure("gt 3D reprojected")
            # print(point_im_cam[0, 0, 0, :, :])
            # h = point_im_cam[0, 0, 0, :, 0]
            # w = point_im_cam[0, 0, 0, :, 1]
            # h = torch.where(h<255, h, 255.)
            # h = torch.where(h > 0, h, 0.)
            # w = torch.where(w < 255, w, 255.)
            # w = torch.where(w > 0, w, 0.)
            # h=h.detach().cpu().numpy()/255
            # w=w.detach().cpu().numpy()/255
            # plt.scatter(h, w)
            #
            # fig = plt.figure("input 2d")
            # h = inp[0, 0,:, 0, 0].detach().cpu().numpy() #[360, 7, 13, 4, 4] (B, T, pts, C, cams)
            # w = inp[0, 0,:, 1, 0].detach().cpu().numpy()
            # plt.scatter(h, w)
            #
            #
            # plt.show()
            unsup_gt = inp[:, pad:pad+1, :, :2, :].cuda()
            point_im_cam = torch.where(point_im_cam < 255, point_im_cam, 255.)
            point_im_cam = torch.where(point_im_cam > 0, point_im_cam, 0.)
            point_im_cam = point_im_cam/255
            """print(point_im_cam[0])
            print(unsup_gt[0])
            input()"""
            unsup_gt = torch.permute(unsup_gt, (0, 1, 4, 2, 3)).contiguous()
            #print("inp.shape : " + str(unsup_gt.shape))
            #print("point_im_cam.shape : " + str(point_im_cam.shape))

            loss = mpjpe(point_im_cam, unsup_gt)




            if summary_writer is not None:
                summary_writer.add_scalar("unsup_loss/iter", loss, iters)
                summary_writer.add_scalar("sup_loss/iter", sup_loss, iters)
        ...
        with torch.no_grad():
            ...
            for t_len in cfg.TEST.NUM_FRAMES:
                    ...
                    for view_list in itertools.combinations(list(range(NUM_VIEW)), num_view):
                        ...
                        for valid_subject in cfg.OBJ_POSE_DATA.SUBJECTS_TEST:
                            ...
                            for act in vis_actions if EVAL else test_actions:
                                ...
                                if EVAL:
                                    test_generator = ChunkedGenerator(cfg.TEST.BATCH_SIZE, poses_valid_2d, 1,pad=pad_t, causal_shift=causal_shift, shuffle=False, augment=False)
                                else:
                                ...
                                for batch_2d, _ in test_generator.next_epoch():
                                    ...
                                    out = out.detach()#.cpu()
                                    #print("out.shape : "+str(out.shape))
                                    out = torch.mean(out, 4, keepdim=True).repeat(1, 1, 1, 1, 4)
                                    ...
                                    if EVAL:
                                        out_ = out.permute(0, 1, 4, 2, 3).contiguous()  # (B, T, N, J. C)
                                        pos_gt_ = pos_gt.permute(0, 1, 4, 2, 3).contiguous()

                                        x_gt = pos_gt_[0, 0, 0, :, 0].detach().cpu().numpy()
                                        y_gt = pos_gt_[0, 0, 0, :, 1].detach().cpu().numpy()
                                        z_gt = pos_gt_[0, 0, 0, :, 2].detach().cpu().numpy()
                                        fig = plt.figure("gt 3D")
                                        ax = plt.axes(projection='3d')
                                        ax.set_box_aspect((np.ptp(x_gt), np.ptp(y_gt), np.ptp(z_gt)))
                                        ax.scatter3D(x_gt, y_gt, z_gt, color='g', marker="+",label="GT_3D")

                                        x_out = out_[0, 0, 0, :, 0].detach().cpu().numpy()
                                        y_out = out_[0, 0, 0, :, 1].detach().cpu().numpy()
                                        z_out = out_[0, 0, 0, :, 2].detach().cpu().numpy()
                                        #ax = plt.axes(projection='3d')
                                        #ax.set_box_aspect((np.ptp(x), np.ptp(y), np.ptp(z)))
                                        ax.scatter3D(x_out, y_out, z_out, color='r', marker="+",label="PRED_3D")
                                        plt.legend()

                                        print("np.mean(x_gt-x_out) : "+str(np.mean(x_gt-x_out)))
                                        print("np.mean(y_gt-y_out) : "+str(np.mean(y_gt-y_out)))
                                        print("np.mean(z_gt-z_out) : "+str(np.mean(z_gt-z_out)))

                                        """print("pos_gt.shape : "+str(pos_gt.shape))
                                        print("out.shape : " + str(out.shape))"""
                                        # print("inp.shape : " + str(inp.shape))
                                        # print(pos_gt[0, 0, 0, :, :])
                                        init_shape = out_.shape
                                        out_unsup = torch.cat((out_, torch.ones(out_.shape[:-1] + (1,)).cuda()), axis=-1)
                                        print("out_unsup.shape : "+str(out_unsup.shape))
                                        out_unsup = torch.permute(out_unsup.view(*out_unsup.shape[:3], -1, 4),
                                                                  (0, 1, 3, 2, 4)).contiguous()
                                        point_im_cam = torch.einsum('voi, btpvi->btpvo', full_transf, out_unsup)
                                        point_im_cam = torch.permute(point_im_cam, (0, 1, 3, 2, 4)).contiguous()
                                        point_im_cam = point_im_cam.view(*init_shape[:-1], 4)
                                        point_im_cam = torch.divide(point_im_cam[:, :, :, :, :2],
                                                                    point_im_cam[:, :, :, :, 2:3])
                                        # print("point_im_cam.shape : "+str(point_im_cam.shape))
                                        cam_idx = 0
                                        for cam_name in ["left_shoulder", "right_shoulder", "overhead", "front"]:

                                            #fig = plt.figure("gt 3D reprojected "+cam_name)
                                            im = plt.imread("./data/RLBench_pose_est_data/ex_240/"+cam_name+"/240_rgb_4.png")
                                            fig = plt.figure("2D cam "+str(cam_name))
                                            implot = plt.imshow(im)
                                            print(point_im_cam[0, 0, cam_idx, :, :])
                                            h = point_im_cam[0, 0, cam_idx, :, 0]
                                            w = point_im_cam[0, 0, cam_idx, :, 1]
                                            h = torch.where(h<255, h, 255.)
                                            h = torch.where(h > 0, h, 0.)
                                            w = torch.where(w < 255, w, 255.)
                                            w = torch.where(w > 0, w, 0.)
                                            ...
                                            plt.scatter(h, w, color='r', marker="+",label="PRED_3D_PROJ_TO_2D")

                                            h = inp[0, 0,:, 0, cam_idx].detach().cpu().numpy()*256 #[360, 7, 13, 4, 4] (B, T, pts, C, cams)
                                            w = inp[0, 0,:, 1, cam_idx].detach().cpu().numpy()*256
                                            plt.scatter(h, w, color='g', marker="+",label="INPUT_2D")
                                            plt.legend()
                                    ...
                                    for idx, view_idx in enumerate(view_list):
                                        ...
                                        print("t_len_id : " + str(t_len_id))
                                        print("view_list_id : " + str(view_list_id))
                                        print("valid_subject_id : " + str(valid_subject_id))

                                        print("len(eval_results) : " + str(len(eval_results)))
                                        print("len(eval_results[t_len_id]) : " + str(len(eval_results[t_len_id])))
                                        print("len(eval_results[t_len_id][num_view_id]) : " + str(len(eval_results[t_len_id][num_view_id])))
                                        print("len(eval_results[t_len_id][num_view_id][view_list_id]) : " + str(len(eval_results[t_len_id][num_view_id][view_list_id])))
                                        print("len(eval_results[t_len_id][num_view_id][view_list_id][valid_subject_id]) : " + str(len(eval_results[t_len_id][num_view_id][view_list_id][valid_subject_id])))"""
                                        ...
                                        eval_results[t_len_id][num_view_id][view_list_id][valid_subject_id][idx].append(loss_view_tmp.cpu())
                ...
                eval_results_center = np.array(eval_results_center)
                eval_results_dims = np.array(eval_results_dims)
                eval_results_quat = np.array(eval_results_quat)"""
                ...
                for task_idx in range(len(cfg.OBJ_POSE_DATA.SUBJECTS_TEST)):
                    ...
                    for i in range(NUM_VIEW):
                        view_mean_res_val = np.mean(np.array(eval_results[0][0][0][task_idx][i]))
                        if summary_writer is not None:
                            summary_writer.add_scalar("test_mpjpe_tsk{}_view{}/epoch".format(tsk, i), view_mean_res_val, epoch)
                        else:
                            print("test_mpjpe_tsk{}_view{} : {}".format(tsk, i, view_mean_res_val))
