...
def mpjpe(predicted, target):
    ...
    """print(torch.norm(predicted - target, dim=len(target.shape)-1))
    input()"""
...
def get_rotation(target):
    ...
    normX = np.sum(X0**2, axis=(1, 2), keepdims=True)
    normY = np.sum(Y0**2, axis=(1, 2), keepdims=True)
    normX = np.tile(np.where(normX>0, np.sqrt(normX), 0), (1, J, C))
    normY = np.tile(np.where(normY>0, np.sqrt(normY), 0), (1, J, C))
    ...
    print(X0.shape)
    print(normX.shape)
    X0 = np.where(normX!=0, X0/normX, 0)
    Y0 = np.where(normY!=0, Y0/normY, 0)
