...
# DATA
config.DATA = edict()
config.DATA.HAS_2D_PREDS = True
config.DATA.USE_GT_2D = False
config.DATA.DATASET_NAME = "obj_pose"
...
### Obj pose estimation Data
config.OBJ_POSE_DATA = edict()
config.OBJ_POSE_DATA.ROOT_DIR = './data/RLBench_pose_est_data'
config.OBJ_POSE_DATA.SUBJECTS_TRAIN = ['T79']#['T0', 'T8', 'T41', 'T84', 'T79']#
config.OBJ_POSE_DATA.SUBJECTS_TEST = ['T79']#['T0', 'T8', 'T41', 'T84', 'T79']#['T0']#
config.OBJ_POSE_DATA.TRAIN_CAMERAS = [0, 1, 2, 3]
config.OBJ_POSE_DATA.TEST_CAMERAS = [0, 1, 2, 3]
config.OBJ_POSE_DATA.NUM_OBJS = 5
config.OBJ_POSE_DATA.DIM_OBJ = 5
config.OBJ_POSE_DATA.NUM_WPS = 6
config.OBJ_POSE_DATA.DIM_WP = 8
config.OBJ_POSE_DATA.IMAGES_PATH=''

#config.OBJ_POSE_DATA.NUM_JOINTS = 5
#config.OBJ_POSE_DATA.DIM_JOINT = 3

config.OBJ_POSE_DATA.PROJ_Frm_3DCAM = False
...
config.TEST.CHECKPOINT = "./home/lambda-ipi/pose3d/pose_obj_3d/checkpoint/submit/mht_consist_gt_trans_t_7_no_res_2022-11-19-21-44/model.bin"
...
if config.DATA.DATASET_NAME =='obj_pose':
    config.NETWORK.INPUT_DIM = 16 # obj_class, xmin, xmax, ymin, ymax
    config.NETWORK.DIM_JOINT = 8 #xmin, xmax, ymin, ymax, zmin, zmax of object shifted to 0rigin + x, y, z of object center (!= data dim joint)
else:
    config.NETWORK.INPUT_DIM = 2 #不考虑confidence的维度
    config.NETWORK.DIM_JOINT = 3
config.NETWORK.USE_GT_TRANSFORM = False
config.NETWORK.TRANSFORM_DIM = 2
config.NETWORK.TEMPORAL_LENGTH = 1
config.NETWORK.TEMPORAL_MASK = []
config.NETWORK.AFTER_MHF_DIM = 256
config.NETWORK.MASK_RATE = 0.4
config.NETWORK.DROPOUT = 0.0
...
config.NETWORK.M_FORMER.NUM_RELATION_LAYERS = 2
...
config.NETWORK.T_FORMER.NUM_HEADS = 4
config.NETWORK.T_FORMER.NUM_CHANNELS = 256
