...
# DATA
config.DATA = edict()
config.DATA.HAS_2D_PREDS = False
...
config.OBJ_POSE_DATA.ROOT_DIR = '/home/lambda-ipi/pose3d/pose_obj_3d/data/RLBench_pose_est_data'
config.OBJ_POSE_DATA.SUBJECTS_TRAIN = ['T0']#['T0', 'T8', 'T41', 'T84', 'T79']#
config.OBJ_POSE_DATA.SUBJECTS_TEST = ['T0']#['T0', 'T8', 'T41', 'T84', 'T79']#['T0']#
config.OBJ_POSE_DATA.TRAIN_CAMERAS = [0, 1, 2, 3]
config.OBJ_POSE_DATA.TEST_CAMERAS = [0, 1, 2, 3]
config.OBJ_POSE_DATA.NUM_JOINTS = 3
config.OBJ_POSE_DATA.DIM_JOINT = 6
...
if config.DATA.DATASET_NAME =='obj_pose':
    config.NETWORK.INPUT_DIM = 5 # obj_class, xmin, xmax, ymin, ymax
    config.NETWORK.DIM_JOINT = 6+3 #xmin, xmax, ymin, ymax, zmin, zmax of object shifted to 0rigin + x, y, z of object center (!= data dim joint)
else:
