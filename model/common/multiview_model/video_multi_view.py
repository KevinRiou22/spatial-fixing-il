...
class RotationModel(nn.Module):
    ...
    def forward(self, x):
        B, _,_, T, _, _ = x.shape
        x = x.contiguous().view(B, x.shape[1] * x.shape[2], T, x.shape[4] * x.shape[5]).contiguous()
        x = self.drop(self.relu(self.expand_bn(self.expand_conv(x))))
...
class Pose3dShrink(nn.Module):
    def __init__(self, cfg, N,channels, dropout = 0.25, momentum = 0.1,is_train = False, num_joints = 17):
        ...
        self.conv_layers = nn.ModuleList(conv_layers)
        self.bn_layers = nn.ModuleList(bn_layers)
        self.shrink = nn.Conv2d(channels, num_joints * cfg.NETWORK.DIM_JOINT, 1)

        self.dim_joint = cfg.NETWORK.DIM_JOINT
...
class FuseView(nn.Module):
    ...
    def mtf_forward(self, x, pos_2d, rotation = None):
        ...
        if self.training and self.Thr > 0:
            mask = torch.rand(B, 1, 1, 1, self.num_view, self.num_view)[...,:N,:N].to(x.device) + self.m[...,:N,:N].to(x.device) #(B, C1, C1, T, N, N)
            mask = mask < self.Thr
...
class Pose3dShrinkOther(nn.Module):
    def __init__(self, cfg, N,channels, dropout = 0.25, momentum = 0.1,is_train = False, num_joints = 17):
        super().__init__()
        self.dim_joint = cfg.NETWORK.DIM_JOINT
        self.conv_1 = nn.Conv2d(N * cfg.NETWORK.TRANSFORM_DIM, channels, (1, 1),stride = (1, 1),dilation = (self.dim_joint**0, 1), bias = False)
        ...
        for i in range(self.num_layers):
            if is_train:
                conv_layers.append(nn.Conv2d(channels, channels, (self.dim_joint,1),stride = (self.dim_joint, 1), bias = False))
            else:
                conv_layers.append(nn.Conv2d(channels, channels, (self.dim_joint,1),stride = (1, 1), dilation = (self.dim_joint**(i+1), 1),bias = False))
            bn_layers.append(nn.BatchNorm2d(channels, momentum=momentum))
            conv_layers.append(nn.Conv2d(channels, channels, 1, bias = False))
            bn_layers.append(nn.BatchNorm2d(channels, momentum=momentum))
        self.conv_layers = nn.ModuleList(conv_layers)
...
class VideoMultiViewModel(nn.Module):
    def __init__(self, cfg, momentum = 0.1,is_train = False,  num_joints = 17, ):
        ...
        if is_train and use_inter_loss:
            self.shrink_1 = Pose3dShrinkOther(cfg, N = N, channels = cfg.NETWORK.NUM_CHANNELS,dropout = cfg.NETWORK.DROPOUT, is_train = is_train, num_joints = num_joints)
            if self.cfg.NETWORK.USE_MFT:
                self.shrink_2 = Pose3dShrinkOther(cfg, N = N, channels = cfg.NETWORK.NUM_CHANNELS, dropout = cfg.NETWORK.DROPOUT, is_train = is_train, num_joints = num_joints)
        ...
        if self.cfg.NETWORK.USE_FEATURE_TRAN and is_train and self.cfg.NETWORK.M_FORMER.MODE == 'mtf':
            if not self.cfg.NETWORK.USE_GT_TRANSFORM:
                if self.cfg.TRAIN.USE_ROT_LOSS and self.training:
                    self.tran_shrink = RotShrink(cfg, N = N, channels = cfg.NETWORK.NUM_CHANNELS, dropout = cfg.NETWORK.DROPOUT, is_train = is_train, num_joints = num_joints)
            else:
