...
class Task(object):
    ...
    def get_objects_annotations(self) -> np.ndarray:
        """Gets the pose and various other properties of objects in the task.
        ...
        state = {"joints":[], "objects":[], "waypoints":{}}
        for obj, objtype in self._initial_objs_in_scene:
            if obj.still_exists():
                if obj.get_type() == ObjectType.SHAPE:
                    no_digits = []
                    for i in obj.get_name():
                        if not i.isdigit():
                            no_digits.append(i)
                    # line_list = stripped_line.split()
                    """print("tip pose : "+str(self.robot.arm.get_tip().get_position()))
                    print("tip orientation : "+str(self.robot.arm.get_tip().get_orientation()))"""
                    no_digits = ''.join(no_digits)
                    bbox_ = obj.get_bounding_box()
                    position_ = obj.get_position()
                    bbox_[0:2] = bbox_[0:2]+position_[0]
                    bbox_[2:4] = bbox_[2:4] + position_[1]
                    bbox_[4:6] = bbox_[4:6] + position_[2]
                    quat_ = obj.get_quaternion()
                    # print(quat_)
                    objt = {"name": no_digits, "bbox": bbox_, "quat": quat_.tolist(), "id":obj.get_name()}
                    state["objects"].append(objt)
                    """print("object : "+str(obj.get_name()))
                    print("position : "+str(obj.get_position()))
                    print("orientation : " + str(obj.get_orientation()))
                    print("bounding_box : " + str(obj.get_bounding_box()))

                    print("---------------------------")"""
                elif obj.get_type() == ObjectType.DUMMY:
                    #print(obj.get_name())
                    if obj.get_name().startswith('waypoint'):
                        #print(obj.get_position())
                        #print(obj.get_quaternion())
                        #print(obj.get_extension_string())
                        parent_obj = obj.get_parent()
                        # while parent_obj.get_type()!=ObjectType.SHAPE:
                        #     parent_obj = parent_obj.get_parent()
                        no_digits = []
                        for i in parent_obj.get_name():
                            if not i.isdigit():
                                no_digits.append(i)
                        parent_class = no_digits
                        parent_id = parent_obj.get_name()
                        objt = {"pos": obj.get_position().tolist(), "quat": obj.get_quaternion().tolist(), "ext_string":obj.get_extension_string(), "parent_class":parent_class, "parent_id":parent_id}
                        state["waypoints"][obj.get_name()]=objt
                """elif obj.get_type() == ObjectType.DUMMY:
                    print("dummy : " + str(obj.get_name()))"""
                """state.extend(np.array(obj.get_pose()))
                if obj.get_type() == ObjectType.JOINT:
                    state.extend([Joint(obj.get_handle()).get_joint_position()])
                elif obj.get_type() == ObjectType.FORCE_SENSOR:
