...
class Scene(object):
    ...
    def get_observation(self) -> Observation:
        ...
        #objects_annotations = self._active_task.get_objects_annotations()
        obs = Observation(
            ...
            gripper_open=(
                self._robot.gripper.get_open_amount()[0]
                if self._obs_config.gripper_open else None),
            gripper_pose=(
                np.array(tip.get_pose())
                if self._obs_config.gripper_pose else None),
            gripper_euler_pose=(
                np.concatenate([np.array(tip.get_position()), np.array(tip.get_orientation())])
                if self._obs_config.gripper_pose else None),
            gripper_matrix=(
                tip.get_matrix()
            ...
            misc=self._get_misc(),
            obj_annotations=self._active_task.get_objects_annotations()
        )
    ...
    def get_demo(self, record: bool = True,
                 ...
                 randomly_place: bool = True, dissociate_reach_close=False) -> Demo:
        ...
        demo = []
        #sub_demos_list = [[] for k in range(len(waypoints))]
        ...
        t=1
        timesteps_sub_demos = []
        print("len(waypoints) : "+str(len(waypoints)))
        init_state = self._initial_task_state
        while True:
            success = False
            for i, point in enumerate(waypoints):
                ...
                while not done:
                    ...
                    #self._demo_record_step(sub_demos_list[i], record, callable_each_step)
                    t+=1
                    success, term = self._active_task.success()
                if dissociate_reach_close:
                    timesteps_sub_demos.append(t)
        ...
        if not success:
            raise DemoError('Demo was completed, but was not successful.',
                            self._active_task)
        return Demo(demo), timesteps_sub_demos
