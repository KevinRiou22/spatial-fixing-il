...
class ObservationConfig(object):
    def __init__(self,
        ...
        self.left_shoulder_camera = (
            CameraConfig(image_size=image_size) if left_shoulder_camera is None
            else left_shoulder_camera)
        self.right_shoulder_camera = (
            CameraConfig(image_size=image_size) if right_shoulder_camera is None
            else right_shoulder_camera)
        self.overhead_camera = (
            CameraConfig(image_size=image_size) if overhead_camera is None
            else overhead_camera)
        self.wrist_camera = (
            CameraConfig(image_size=image_size) if wrist_camera is None
            else wrist_camera)
        self.front_camera = (
            CameraConfig(image_size=image_size) if front_camera is None
            else front_camera)
