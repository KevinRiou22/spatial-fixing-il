...
class TaskEnvironment(object):
    ...
    def __init__(self, pyrep: PyRep, robot: Robot, scene: Scene, task: Task,
                 ...
                 attach_grasped_objects: bool = True, enable_path_observations=True):
        self._pyrep = pyrep
        self._robot = robot
        self._scene = scene
        self._task = task
        ...
        self._enable_path_observations = enable_path_observations
    ...
    def _ee_action(self, action, last_obs, relative_to=None):
        ...
        while not done:
            self._scene.step()
            last_obs = last_obs[1:]
            last_obs.append(self._scene.get_observation())
    ...
    def step_several_ts_obs(self, action, n_last_obs=1) -> (Observation, int, bool):
        ...
        last_obs = [None for k in range(n_last_obs)]
        if ee_action > 0.5:
            ee_action = 1.0
        elif ee_action < 0.5:
            ee_action = 0.0
        ...
        if self._action_mode.arm == ArmActionMode.ABS_JOINT_VELOCITY:
            ...
            last_obs=self._ee_action(list(arm_action), last_obs)
            ...
            qw, qx, qy, qz = list(new_rot)
            new_pose = [a_x + x, a_y + y, a_z + z] + [qx, qy, qz, qw]
            last_obs = self._ee_action(list(new_pose), last_obs)

        elif self._action_mode.arm == ArmActionMode.EE_POSE_EE_FRAME:

            self._assert_action_space(arm_action, (7,))
            last_obs = self._ee_action(list(arm_action), last_obs, relative_to=self._robot.arm.get_tip())
        ...
        return last_obs, reward, terminate

    def step(self, action, return_path_obs=False) -> (Observation, int, bool):
        ...
        reward = float(success) if task_reward is None else task_reward
        if return_path_obs:
            return self._scene.get_observation(), reward, terminate, self._path_observations
        else:
    ...
    def _get_live_demos(self, amount: int,
        ...
        for i in range(amount):
            ...
            while attempts > 0:
                ...
                self.reset()
                try:
                    demo, timesteps_waypoints = self._scene.get_demo(
                        callable_each_step=callable_each_step)
                    demo.random_seed = random_seed
                    print("demo.random_seed : "+str(demo.random_seed))
                    demos.append(demo)
                    all_timesteps_waypoints.append(timesteps_waypoints)
                    break
